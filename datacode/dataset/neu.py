...
class SegDataset(Dataset):
    ...
    def __init__(self, root ='/media/aries/Udata/defect/NEU_Seg-main',
        ...
        self.img_mean=[0.44545, 0.44545, 0.44545]
        self.img_std=[0.205742, 0.205742, 0.205742]
    ...
    def __getitem__(self, index):
        ...
        image = cv2.imread(join(self.root, 'images', self.split, '{}.jpg'.format(name)))
        label = cv2.imread(join(self.root, 'annotations', self.split, '{}.png'.format(name)), 0)  ##读成1通道
        #eval直接返回
        if  self.split=='test':
            image = cv2.resize(image, (self.cropsize, self.cropsize), interpolation=cv2.INTER_LINEAR)
            label = cv2.resize(label, (self.cropsize, self.cropsize), interpolation=cv2.INTER_NEAREST)
            return self.transform(image),label,image
        image = image[:, :, ::-1]#读取为RGB
        # resize处理
        ...
        # if self.imagesize==self.cropsize:
        #     offsetx=0
        #     offsety=0
        # else:
