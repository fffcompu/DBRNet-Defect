...
class SegDataset(Dataset):
    ...
    def __init__(self, root ='/media/aries/Udata/defect/NEU_Seg-main',
        ...
        self.split = split
        lines = open(join(root,'ImageSets','Segmentation','{}.txt'.format(self.split)), 'r').readlines()
        self.samples = []
        self.root = root
    ...
    def __getitem__(self, index):
        ...
        image = cv2.imread(join(self.root, 'JPEGImages', '{}.jpg'.format(name)))
        label = cv2.imread(join(self.root, 'SegmentationClass','{}.png'.format(name)), 0)  ##读成1通道
